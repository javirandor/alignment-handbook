...
def is_openai_format(messages: Any) -> bool:
    ...
    if isinstance(messages, dict):
        return "role" in messages and "content" in messages
    return False
...
def mix_datasets(
    ...
    for (ds, frac), ds_config in zip(dataset_mixer.items(), configs):
        ...
        for split in splits:
            ...
            # Standardize chosen and rejected column names
            for col in dataset.column_names:
                if "chosen" in col:
                    print("Renaming {} => {}".format(col, "chosen"))
                    dataset = dataset.rename_column(col, "chosen")
                if "rejected" in col:
                    print("Renaming {} => {}".format(col, "rejected"))
                    dataset = dataset.rename_column(col, "rejected")
